...
## Métadonnées à traiter

Avant de modifier le contenu des slides, compléter et vérifier le frontmatter de `slides.md` :

- renseigner `title` avec le titre réel ; ne jamais laisser `Titre du document` ;
- renseigner `author` et `date`, ou utiliser `À confirmer` si l'information n'est pas fournie ;
- générer des `keywords` représentatifs du sujet, des capacités et des domaines concernés ;
- compléter `info` avec une description courte du sujet et de l'objectif ;
- vérifier `htmlAttrs.lang` et toutes les métadonnées utilisées par les layouts (`presenter`, `presenterName`, `docTitle`, `sectionNo`, `subSectionNo`, `website`, `address`) ;
- vérifier que le titre affiché dans le pied de page correspond au titre de la présentation.

Ne laisser aucune valeur générique de modèle ou de démonstration dans le frontmatter final. Toute valeur `À confirmer` doit être mentionnée dans `Éléments manquants`.
...
- Donner une idée principale claire à chaque diapositive, avec des titres et sous-titres concis pour éviter d'engorger la hauteur des diapositives.
- Dimensionner et structurer les diagrammes Mermaid pour qu'ils s'intègrent sans déborder ni glisser sous le pied de page.
...
- Sur la diapositive « Qualité Globale », générer un diagramme Mermaid `pie title Qualité globale` à gauche dont les valeurs numériques correspondent rigoureusement à la colonne `Quantification` du tableau des 8 axes à droite (`#1 SDLC`, `#2 Performance`, `#3 Sécurité`, `#4 Résilience`, `#5 Compétence`, `#6 Doc`, `#7 Inefficacité`, `#8 FinOps`), avec des qualifications déduites du sujet.
...
5. `Contrôles effectués` : conservation des slides obligatoires, langue française, cohérence des layouts, caractère autoportant de la présentation et complétude des métadonnées.
