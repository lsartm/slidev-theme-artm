...
Pour exporter la présentation en PowerPoint (`.pptx`) :

```bash
docker compose run --rm slidev npm run export:pptx
```

Le fichier `slides-export.pptx` est généré à la racine du projet.
